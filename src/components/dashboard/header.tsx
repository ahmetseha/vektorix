...
export function Header({ onMenuClick }: { onMenuClick: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-header items-center gap-3 border-b border-surface-border bg-surface/90 px-4 backdrop-blur-xl sm:px-6">
      ...
      <nav aria-label="Breadcrumb" className="hidden min-w-0 items-center gap-2 text-sm sm:flex">
        <span className="text-text-subtle">Dashboard</span><span className="text-text-subtle">/</span><span className="truncate font-medium text-text-main">{current}</span>
      </nav>
      <div className="ml-auto flex items-center gap-2 sm:gap-3">
        <div className="relative hidden w-72 lg:block">
          <Search className="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-text-subtle" />
          <Input ref={searchRef} placeholder="Search..." aria-label="Search" className="bg-background pr-14 pl-9" />
