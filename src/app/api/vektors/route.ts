...
export async function POST(request: Request) {
  ...
  if (!allowRequest(`publish:${client}`, 8, 60_000)) return NextResponse.json({ error: "Too many creations. Let the field settle." }, { status: 429 });
  const length = Number(request.headers.get("content-length") ?? 0);
  if (length > 1_600_000) return NextResponse.json({ error: "Payload is too large." }, { status: 413 });
  try {
    const input = publishVektorSchema.parse(await request.json());
    const prohibited = /\b(?:fuck|shit|cunt|nazi)\b/i;
    if (prohibited.test(input.name) || prohibited.test(input.description)) {
      return NextResponse.json({ error: "Choose a different name or field note." }, { status: 400 });
    }
