...
import { vektorDnaSchema } from "../schemas/dna";
...
const archetypeBias: Record<Archetype, { speed: number; turbulence: number; pulse: number; orbit: number }> = {
...
export function createDNA({
  ...
  const jitter = (center: number, amount = 0.12) => clamp(center + randomBetween(random, -amount, amount));
  ...
  return vektorDnaSchema.parse({
    ...
    surface: {
      roughness: archetype === "electric" ? randomBetween(random, 0.14, 0.36) : randomBetween(random, 0.28, 0.72),
      metallic: archetype === "electric" ? randomBetween(random, 0.45, 0.82) : randomBetween(random, 0.08, 0.48),
    ...
    particles: {
      countTier: "medium",
      spread: jitter(0.32 + Math.max(0, gesture.directionBias) * 0.5),
      trailLength: jitter(0.28 + gesture.averageSpeed * 0.6),
      attraction: jitter(0.52 + Math.max(0, -gesture.directionBias) * 0.35),
      repulsion: jitter(0.2 + Math.max(0, gesture.directionBias) * 0.55),
    },
    audio: {
...
export function encodeDNA(dna: VektorDNA): string {
  const json = JSON.stringify(vektorDnaSchema.parse(dna));
...
export function decodeDNA(value: string): VektorDNA {
  ...
  return vektorDnaSchema.parse(JSON.parse(json));
}
