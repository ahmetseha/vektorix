...
export function summarizeGesture(points: GesturePoint[]): GestureSummary {
  ...
  for (let index = 1; index < points.length; index += 1) {
    ...
    length += distance;
    directionTotal += dx;
    radialTotal += Math.hypot(current.x, current.y) - Math.hypot(previous.x, previous.y);
  ...
  const activeDirections = signature.filter((value) => value >= 0.16).length;
  ...
  return {
    ...
    radialBias: clamp(radialTotal / Math.max(length, 0.0001), -1, 1),
    variety: clamp(activeDirections / signature.length),
