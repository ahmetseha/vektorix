...
export function ExploreGrid() {
  ...
  const items = useMemo(() => {
    ...
    if (["Calm", "Chaotic", "Electric", "Organic"].includes(filter)) return result.filter((item) => item.dna.visual.archetype === filter.toLowerCase());
  ...
  return (
    <>
      ...
      <div className="explore-grid">{items.map((record, index) => <Link className="explore-card" key={record.slug} href={`/v/${record.slug}?data=${encodeDNA(record.dna)}`}><span className="card-index">{String(index + 1).padStart(2, "0")}</span>{record.previewImageUrl ? <img src={record.previewImageUrl} alt={`Preview of ${record.name}`} /> : <OrganismPoster dna={record.dna} label={`Preview of ${record.name}`} />}<div className="card-meta"><span>{record.dna.visual.archetype}</span><h2>{record.name}</h2><p>{record.creator}</p><dl><div><dt>GEN</dt><dd>{record.generation}</dd></div><div><dt>REMIX</dt><dd>{record.remixCount}</dd></div></dl><ArrowUpRight /></div></Link>)}</div>
